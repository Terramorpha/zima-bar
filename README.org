...
#+language: en
#+begin_quote
Zima Blue, the manufacturer called it. The first thing I ever saw. 
...
unexpressive implementation of half of scheme". The bar widget I made could very well have been a guile scheme program. I went and found =g-golf=,
the gobject bindings for guile and implemented the status bar.

However, after I had a complete implementation of what I wanted (it worked using
the main thread for the gtk rendering loop, plus a second thread watching the
events and modifying the bar through ~g_idle_add~ callbacks), I found this two-threads hack
unsatisfying.
...
the entire IO system to support asynchronous IO in the style of event loops such as GLib or libuv. This is the trick I performed and you can now observe the results: a single threaded status bar made by gutting guile's direct style IO and stuffing GLib's event loop back in.

May this status bar, in honor of Zima Blue, metastasize into a complex mass of unjustifiable sophistication.
